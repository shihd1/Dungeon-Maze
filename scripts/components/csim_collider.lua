...
function csim_collider:udpateHorizontal()
    ...
    col_pos1.x = col_pos1.x + parent_rigid_body.vel.x + (8 * horiz_side)
    col_pos2.x = col_pos2.x + parent_rigid_body.vel.x + (8 * horiz_side)
...
function csim_collider:updateVertical()
    ...
    col_pos1.y = col_pos1.y + parent_rigid_body.vel.y + (8 * vert_side)
    col_pos2.y = col_pos2.y + parent_rigid_body.vel.y + (8 * vert_side)
...
function csim_collider:detectVerticalCure(tile_x, tile_y, vert_side)
  -- TODO: Create a variable to store the tile which the object is trying to visit
  -- Hint: Use map.layers['Terrain']
  local map_data = map.layers["Terrain"].data
  if map_data[tile_y+1] and map_data[tile_y+1][tile_x+1] then
      local tile = map_data[tile_y+1][tile_x+1]
      if (tile) then
          if(tile.properties["cure"]) then
              csim_debug.text("cure")
              return true
          end
      end
  end
...
function csim_collider:detectTrap(tile_x, tile_y)
  local map_data = map.layers["Terrain"].data
  if map_data[tile_y+1] and map_data[tile_y+1][tile_x+1] then
      local tile = map_data[tile_y+1][tile_x+1]
      if (tile) then
          if(tile.properties["Trap"]) then
              csim_debug.text("Trap")
              return true
          end
      end
  end
...
function csim_collider:didCollideVertically(tile_x, tile_y, vert_side)
    ...
    if(vert_side == -1) then
        self.parent.pos.y = screen_y + map.tileheight - self.rect.y + 8
    elseif(vert_side == 1) then
        self.parent.pos.y = screen_y - self.rect.y - self.rect.h - 8
    end
end
...
function csim_collider:detectHorizontalCure(tile_x, tile_y, vert_side)
  local map_data = map.layers["Terrain"].data
  if map_data[tile_y+1] and map_data[tile_y+1][tile_x+1] then
      local tile = map_data[tile_y+1][tile_x+1]
      if (tile) then
...
function csim_collider:didCollideHorizontally(tile_x, tile_y, horiz_side)
    ...
    screen_x, screen_y = map:convertTileToPixel(tile_x, tile_y)
    if(horiz_side == -1) then
        self.parent.pos.x = screen_x + map.tilewidth - self.rect.x + 8
    elseif(horiz_side == 1) then
        self.parent.pos.x = screen_x - self.rect.x - self.rect.w - 8
    end
end
